...
from .AdvancedReportGenerator import AdvancedReportGenerator, IntegratedSynthesisManager
...
class SinistreAnalysisPipeline:
    ...
    def __init__(self, config: Dict[str, Any] = None):
        ...
        # Initialisation des composants EXISTANTS (gardez tel quel)
        self.data_loader = DataLoader()
        self.data_filter = DataFilter(config.get('filter_params', {}))
        self.preprocessor = DataPreprocessor()
        self.variable_manager = VariableManager()        
        #self.modeling_manager = ModelingManager(config.get('modeling', {}))
        self.stats_manager = StatisticsManager(config.get('stats_params', {}))
        self.visualizer = DataVisualizer()
        
        # MODIFICATION ICI - Remplacez SynthesisManager par IntegratedSynthesisManager
        output_dir = config.get('output_dir', './outputs')
        self.synthesis_manager = IntegratedSynthesisManager(output_dir)
        
        # NOUVEAU - Configuration pour rapports avancés
    ...
    def create_visualizations(self, key_variables: List[str] = None) -> 'SinistreAnalysisPipeline':
        ...
        # NOUVEAU - Intégrer les figures dans le générateur de rapports avancés
