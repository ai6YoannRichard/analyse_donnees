...
class DataFilter:
    ...
    def apply_quality_filters(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        if 'est_sinistre_cas_valide' in filtered_df.columns:
            initial_rows = len(filtered_df)
            filtered_df['est_sinistre_cas_valide'] = self._boolify(filtered_df['est_sinistre_cas_valide'])
            filtered_df = filtered_df[filtered_df['est_sinistre_cas_valide'] == True]
        ...
        if 'montant_charge_brut' in filtered_df.columns:
            initial_rows = len(filtered_df)
            min_montant = self.config.get('min_montant', 100)
