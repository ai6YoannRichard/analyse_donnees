...
class DroughtClaimsAnalyzer(SinistreAnalysisPipeline):
    ...
    def run_comprehensive_drought_analysis(self, file_path: str, **kwargs) -> Dict[str, Any]:
        """
        Lance une analyse complète spécialisée sécheresse

        Args:
            file_path: Chemin vers le fichier de données
            **kwargs: paramètres additionnels (ex: common_load_params=..., preprocess_params=...)
        """
        ...
        # 1) Chargement
        try:
            load_params = kwargs.get('common_load_params', {})
            if hasattr(self, 'load_data') and callable(getattr(self, 'load_data')):
                self.load_data(file_path, **load_params)
                df_raw = getattr(self, 'raw_data', None)
            else:
                # fallback simple
                if file_path.lower().endswith(('.parquet', '.pq')):
                    df_raw = pd.read_parquet(file_path)
                else:
                    df_raw = pd.read_csv(file_path, sep=load_params.get('sep', ','), encoding=load_params.get('encoding', 'utf-8'))
                self.raw_data = df_raw
            if df_raw is None or len(df_raw) == 0:
                raise ValueError("Aucune donnée chargée")
            results['loaded_rows'] = len(df_raw)
        except Exception as e:
            logger.exception("Échec du chargement")
            raise

        # 2) Prétraitement
        try:
            preprocess_params = kwargs.get('preprocess_params', {})
            if hasattr(self, 'preprocess_data') and callable(getattr(self, 'preprocess_data')):
            ...
            if df is None or len(df) == 0:
                raise ValueError("Prétraitement vide")
        except Exception as e:
            logger.exception("Échec du prétraitement")
            raise

        # 3) Statistiques génériques
        self.stats_manager = StatisticsManager(self.config.get('stats_params', {}))
        try:
            self.stats_manager.descriptive_statistics(df, target_var)
            # Analyse temporelle générique
            date_candidates = ['date_exercice', 'date_sinistre', 'date_declaration', 'date']
            date_col = next((c for c in date_candidates if c in df.columns), None)
            if date_col:
                self.stats_manager.temporal_analysis(df, target_var=target_var, date_col=date_col)

                # 🔹 Série mensuelle au format mmaaaa (sum par défaut)
                if hasattr(self.stats_manager, 'plot_monthly_series_mmaaaa'):
                    self.stats_manager.plot_monthly_series_mmaaaa(
                        df, target_var=target_var, date_col=date_col,
                        how="sum", figure_name="serie_mensuelle_mmaaaa", show_counts=True
                    )

            # Corrélations (sur tout le numérique)
            self.stats_manager.correlation_analysis(df, target_variable=target_var, threshold=0.05)

            # Outliers avancés
            self.stats_manager.advanced_outlier_detection(df, target_var=target_var)

            results['statistics_summary'] = self.stats_manager.get_statistics_summary()
        except Exception:
            logger.exception("Échec des statistiques de base")

        # 4) Analyses spécialisées
        try:
            climate = self.analyze_climate_impact()
            vuln = self.analyze_building_vulnerability()
            exposure = self.analyze_risk_exposure()
            segmentation = self.create_risk_segmentation()
            insights = self.generate_drought_specific_insights()
            figures = self.create_advanced_visualizations()
            ...
            logger.exception("Échec des analyses spécialisées")

        # 5) Rapports avancés (si l’infrastructure de synthèse est disponible)
        try:
            if self.config.get('generate_advanced_reports', True) and hasattr(self, 'synthesis_manager'):
                # La synthesis_manager doit être une IntegratedSynthesisManager
                pack = self.synthesis_manager.create_comprehensive_analysis(
                    df=df,
                    target_variable=target_var,
                    stats_manager=self.stats_manager,
                    data_filter=kwargs.get('data_filter'),
                    preprocessor=kwargs.get('preprocessor_summary')
                )
                results['reports'] = pack
        except Exception:
            logger.exception("Échec de la génération des rapports avancés")

        logger.info("✅ Analyse complète sécheresse terminée")
        return results
